...
def crud_api_delete(short_code):
    ...
    match request.method:
        case 'GET':
            if short_code not in dict: return f'Short code: {short_code} was not found!'
            return f'Short code: {short_code} maps to:{dict[short_code]}'
        case 'DELETE':
            if(not is_valid_api_key):  return 'API-KEY is not valid!'
            if short_code not in dict: return f'short-code: {short_code} NOT FOUND'
            dict.pop(short_code) # delete the url short code
        ...
        case 'POST':
            ...
            op = 'UPDATED' if code in dict else 'CREATED'
            dict[code] = url
